...
with abas[3]:
    ...
    if not aq.empty:
        ...
        last = aq.sort_values('data').groupby('municipio', as_index=False).tail(1) if 'data' in aq.columns else aq.copy()
        map_df, geojson_mun, shp_status = prepare_map_dataframe(last)
        st.caption(shp_status)
        fig_aq = make_choropleth_or_points(
            map_df,
            geojson_mun,
            color_col='indice_qualidade_ar_operacional' if 'indice_qualidade_ar_operacional' in map_df.columns else 'iq_ar_score',
            title='Qualidade do ar por município (shapefile)',
            hover_cols=[c for c in ['qualidade_ar_nivel', 'poluente_dominante'] if c in map_df.columns],
            height=500,
            zoom=4.3,
        )
        if fig_aq is not None:
            st.plotly_chart(fig_aq, use_container_width=True)
        elif {'lat', 'lon'}.issubset(aq.columns):
            st.plotly_chart(
                px.scatter_mapbox(
                    last,
                    lat='lat',
                    lon='lon',
                    size='indice_qualidade_ar_operacional' if 'indice_qualidade_ar_operacional' in last.columns else None,
                    color='qualidade_ar_nivel' if 'qualidade_ar_nivel' in last.columns else None,
                    hover_name='municipio' if 'municipio' in last.columns else None,
                    zoom=4.3,
                    height=500,
                    title='Último nível de qualidade do ar por município',
                ),
                use_container_width=True,
            )
        ...
        if 'municipio' not in geo_plot.columns:
            for _col_mun in ['municipio_x', 'municipio_y', 'municipio_base', 'municipio_indicasus']:
                if _col_mun in geo_plot.columns:
                    ...
                    break
        if 'municipio' not in geo_plot.columns:
            geo_plot['municipio'] = geo_plot['cod_ibge'].astype(str) if 'cod_ibge' in geo_plot.columns else 'Município não identificado'
        if 'indice_vulnerabilidade_calor' in geo_plot.columns:
            geo_plot['indice_vulnerabilidade_calor'] = pd.to_numeric(
                geo_plot['indice_vulnerabilidade_calor'], errors='coerce'
            ).fillna(0)
        map_df, geojson_mun, shp_status = prepare_map_dataframe(geo_plot)
        st.caption(shp_status)
        fig_vuln = make_choropleth_or_points(
            map_df,
            geojson_mun,
            color_col='indice_vulnerabilidade_calor',
            title='Vulnerabilidade ao calor (shapefile)',
            height=500,
            zoom=4.3,
        )
        if fig_vuln is not None:
            st.plotly_chart(fig_vuln, use_container_width=True)
        else:
            st.info('Sem shapefile/polígonos ou lat/lon para mapa de vulnerabilidade.')
    else:
