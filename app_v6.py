...
from sisclima.core.db import backend_name, read_table, table_count as db_table_count, table_exists
...
def choropleth_or_points(
    ...
    if fig is None:
        if geojson is None:
            ...
            st.info("Sem dados geográficos suficientes para este mapa.")
        return
...
for df in [met, aq, occ, press, stock, infra, ops_cnes, saude_calor_mun, gal_pos_mun, sim_obitos_mun, alerta_mun_v6, pred_v6]:
...
# ---------------------------------------------------------------------
with tabs[8]:
    st.subheader("Inteligência preditiva e alertas")

    if alerta_mun_v6.empty:
        st.info("Tabela alerta_inteligente_municipal_v6 ainda não criada. Rode inteligencia_alertas_sis_v6.py.")
    else:
        ...
        c1, c2 = st.columns(2)
        with c1:
            make_bar(am, "municipio", "alerta_inteligente_score", "Alerta inteligente municipal - Top")
        with c2:
            make_bar(am, "municipio", "probabilidade_escalada_operacional_pct", "Probabilidade de escalada operacional - Top")
        ...
        if "alerta_inteligente_nivel" in am.columns:
            ...
            choropleth_or_points(
                ...
                "Mapa do alerta inteligente preditivo",
                hover_cols=["regional_saude", "nivel", "score", "alerta_inteligente_score", "risco_cumulativo_3d", "ocupacao_leitos_pct", "pressao_calor_pct"],
                categorical=True,
            )
        ...
        show_df(
            ...
            [
                "cod_ibge", "municipio", "regional_saude", "alerta_inteligente_nivel",
                "alerta_inteligente_score", "score_climatico_preditivo",
                "score_assistencial_preditivo", "score_ar_preditivo",
                "risco_cumulativo_3d_max_7d", "ocupacao_leitos_pct",
                "pm25_ugm3", "recomendacao_operacional",
            ],
            height=520,
        )

    st.markdown("#### Alerta regional")
    show_df(alerta_reg_v6, height=300)

    st.markdown("#### Predição municipal 7 dias")
    show_df(pred_v6, height=360)
