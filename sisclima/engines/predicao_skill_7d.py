...
def nivel_pred_from_agg(row: pd.Series | dict) -> str:
    """Limiares canônicos da predição 7d (mesma regra do enrichment)."""
    score = 0
    r = row.get("risco_cumulativo_3d_max_7d")
    ...
    if t is not None and pd.notna(t):
        ...
        if tv >= 40:
            ...
            score = max(score, 1)
    # LEVEL_ORDER: cinza … roxa; score 0→verde
    return LEVEL_ORDER[min(score + 1, len(LEVEL_ORDER) - 1)]
