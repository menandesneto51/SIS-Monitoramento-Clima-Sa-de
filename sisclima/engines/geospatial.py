    ...
    if geojson is not None and "cod_ibge" in plot_base.columns:
        ...
        for fn_name, style_key in (("choropleth_map", "map_style"), ("choropleth_mapbox", "mapbox_style")):
            ...
            if fn is None:
                continue
            try:
                fig = fn(**common, **{style_key: "carto-positron"}, **extra_cat)
                break
            except Exception as exc:
                log.warning("Falha %s: %s", fn_name, exc)
                fig = None
        if fig is not None:
    ...
    if allow_points_fallback and {"lat", "lon"}.issubset(plot_base.columns):
        ...
        for fn_name, style_key in (("scatter_map", "map_style"), ("scatter_mapbox", "mapbox_style")):
            ...
            if fn is None:
                continue
            try:
                point = fn(plot_df, **kw, **{style_key: "carto-positron"})
                break
            except Exception as exc:
                log.warning("Falha %s: %s", fn_name, exc)
        if point is None:
            return None
        point.update_layout(margin={"r": 0, "t": 50, "l": 0, "b": 0})
        return point
