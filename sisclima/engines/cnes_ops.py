...
def build_ops_cnes_municipio(resumo: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _num_col(name: str) -> pd.Series:
        if name not in mun.columns:
            return pd.Series(np.nan, index=mun.index, dtype="float64")
        return pd.to_numeric(mun[name], errors="coerce")

    pop = _num_col("populacao")
    leitos_tot = _num_col("cnes_leitos_total").fillna(0)
    estab_tot = _num_col("cnes_estabelecimentos_total").fillna(0)
    ...
    score_uti = _num_col("flag_uti").fillna(0).clip(0, 1) * 15
    ...
    ops = mun.copy()
    ocup = pd.to_numeric(ops["ocupacao_leitos_pct"], errors="coerce") if "ocupacao_leitos_pct" in ops.columns else pd.Series(np.nan, index=ops.index)
    ...
    pressao = pd.to_numeric(ops["pressao_calor_pct"], errors="coerce") if "pressao_calor_pct" in ops.columns else pd.Series(0.0, index=ops.index)
