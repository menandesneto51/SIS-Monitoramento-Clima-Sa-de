...
def _coalesce_pred_cols(base: pd.DataFrame) -> pd.DataFrame:
    """Une nivel_predicao_7d / _x / _y após merges duplicados."""
    ...
    for c in candidates[1:]:
        merged = merged.where(merged.notna() & ~merged.astype(str).str.lower().isin(["", "nan", "cinza", "none"]), out[c])
        # prefer non-cinza when available
...
def _merge_base(
    ...
    if pred is not None and not pred.empty and "cod_ibge" in pred.columns:
        ...
        # evita nivel_predicao_7d_x/_y: sobrescreve só se a coluna atual estiver vazia/cinza
...
def _predicao_from_rows(df: pd.DataFrame) -> dict[str, str]:
    ...
    if "nivel_predicao_7d" in work.columns:
        ...
        # moda (mais frequente) para texto complementar
    ...
    # se há tendência de piora mas pred ficou cinza, não mascarar
    ...
    return {
        ...
        "resumo": (
            f"Horizonte de cerca de 7 dias: classificação mais grave prevista "
            f"{LEVEL_LABEL.get(niv, niv)}; classificação mais frequente "
            f"{LEVEL_LABEL.get(moda, moda)}. "
...
def _ocupacao_estadual(df: pd.DataFrame) -> tuple[float | None, str]:
    """Ocupação ponderada por leitos (não média aritmética municipal)."""
    ...
    if lo is not None and lt is not None and float(lt.fillna(0).sum()) > 0:
        ...
        return pct, "ponderada por leitos (ocupados/existentes no estado)"
    try:
        ...
        if table_exists("hospital_ocupacao_estado"):
            ...
            if est is not None and not est.empty and "ocupacao_pct" in est.columns:
                return float(pd.to_numeric(est["ocupacao_pct"], errors="coerce").iloc[-1]), "IndicaSUS estadual (tempo real)"
    except Exception:
        pass
    if "ocupacao_leitos_pct" in df.columns:
        return float(pd.to_numeric(df["ocupacao_leitos_pct"], errors="coerce").mean()), "média municipal (aproximação)"
    return None, "sem dado"


def _indicadores_agregados(df: pd.DataFrame) -> list[dict[str, str]]:
    """Indicadores estaduais com valor + escala/limiar (não usa município sentinela cru)."""
    if df.empty:
        return []
    inds: list[dict[str, str]] = []
    inds.append({"campo": "n_municipios", "rotulo": "Municípios no escopo", "valor": str(len(df)), "escala": "contagem", "limiar": "—"})
    ...
    score_max = int(pd.to_numeric(df.get("score"), errors="coerce").max()) if "score" in df.columns else None
    if score_max is not None and not pd.isna(score_max):
        inds.append(
            {
                "campo": "score",
    ...
    for col, rotulo, escala, limiar, agg in [
        ...
        (
            ...
            "0 a 15 (proxy) ou % real de atendimentos",
        ...
        (
            ...
            "µg/m³",
            "referência OMS diária ~15 µg/m³; valores altos elevam risco respiratório",
            "max",
        ),
        (
            "incidencia_arbovirus_100k",
            "Incidência de arboviroses (pico) por 100 mil",
            "casos / 100 mil hab.",
            "interpretar com tendência e z-score epidêmico",
            "max",
        ),
        (
            "casos_srag",
...
def _motivo_em_linguagem_clara(raw: str) -> str:
    """Remove jargão técnico do motivo exibido no alerta."""
    import re as _re
...
def build_alertas_multinivel(
    ...
    sort_cols = ["_rank"] + (["score"] if "score" in base.columns else [])
    top = base.sort_values(sort_cols, ascending=False).head(15)
    est["municipios_prioritarios"] = [
        {
