...
def sivep_summary(df: pd.DataFrame, populacao: pd.DataFrame | None = None) -> pd.DataFrame:
    """Resumo SIVEP/SRAG com indicadores alinhados ao MS.

    Mantém colunas legadas (casos_srag, uti, obitos, letalidade_pct, zscore_srag)
    e acrescenta incidência, proporção UTI, cobertura lab e vírus dominante.
    """
    try:
        from sisclima.engines.sivep_ms_indicators import sivep_ms_daily_summary

        return sivep_ms_daily_summary(df, populacao=populacao)
    except Exception:
        # Fallback legado se o módulo MS falhar
        ...
        keys = _standard_keys(
            out,
            date_candidates=[
                "data_sintomas", "dt_sin_pri", "data", "data_notificacao", "dt_notific",
                "data_internacao", "DT_NOTIFIC", "DT_INTERNA", "DT_SIN_PRI",
            ],
        ...
        g = clean.groupby(_group_keys(clean), as_index=False).agg(
            casos_srag=("data", "size"), uti=("uti", "sum"), obitos=("obito", "sum")
        )
        ...
        if mcols:
            ...
            g["zscore_srag"] = g.groupby(mcols, group_keys=False)["casos_srag"].transform(
                lambda s: zscore_series(s).fillna(0)
            )
        else:
...
_ARBOVIRUS_PATTERNS: list[tuple[str, str]] = [
    (r"DENGUE|DENV", "Dengue"),
    (r"\bZIKA\b|ZIKV", "Zika"),
    (r"CHIKUNGUNYA|CHIKV|CHIK", "Chikungunya"),
    (r"FEBRE\s*AMARELA|\bFA\b|YELLOW\s*FEVER", "Febre Amarela"),
    (r"OROPOUCHE|OROV", "Oropouche"),
    (r"MAYARO|MAYV", "Mayaro"),
    (r"ARBOVIROSE|ARBOVIRUS|WEST\s*NILE|FEBRE\s*DO\s*NILO", "Outras arboviroses"),
]


def normalize_agravo_arbovirus(text: pd.Series | str) -> pd.Series:
    """Normaliza nomes de agravos para o conjunto canônico de arboviroses."""
    ...
    base = _to_text(s)
    ascii_txt = _strip_accents(base).str.upper()
    out = pd.Series([""] * len(ascii_txt), index=ascii_txt.index, dtype="object")

    for pattern, label in _ARBOVIRUS_PATTERNS:
        mask = ascii_txt.str.contains(pattern, regex=True, na=False) & out.eq("")
        out.loc[mask] = label

    # Mantém o texto original quando não for arbovirose reconhecida.
    out = out.where(out.ne(""), base)
    return out.iloc[0] if single else out


def is_arbovirus(text: pd.Series | str) -> pd.Series:
    norm = normalize_agravo_arbovirus(text)
    if isinstance(norm, str):
        return pd.Series([norm in ARBOVIRUS_CANONICAL])
    return norm.isin(ARBOVIRUS_CANONICAL)
...
def sinan_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    agravo_raw = _as_series(out, ["agravo", "Agravo", "fonte_sinan"], "SINAN")
    agravo_raw = _to_text(agravo_raw).replace({"": "SINAN", "nan": "SINAN", "None": "SINAN", "NaT": "SINAN"})
    agravo = normalize_agravo_arbovirus(agravo_raw)

    clean = keys.copy()
    clean["agravo"] = agravo.loc[clean.index].values
    clean["grupo_agravo"] = np.where(
        clean["agravo"].isin(list(ARBOVIRUS_CANONICAL)),
        "arbovirose",
        "outros",
    )

    g = (
        clean.groupby(_group_keys(clean, extras=["agravo", "grupo_agravo"]), as_index=False)
        .size()
        .rename(columns={"size": "notificacoes"})
    )

    mcols = _municipality_key(g)
    sort_cols = [c for c in (mcols + ["agravo", "data"]) if c in g.columns]
    if sort_cols:
        g = g.sort_values(sort_cols)

    if mcols and "agravo" in g.columns:
        g["zscore_notificacoes"] = (
            g.groupby(mcols + ["agravo"], group_keys=False)["notificacoes"]
            .transform(lambda s: zscore_series(s).fillna(0))
        )
    else:
        g["zscore_notificacoes"] = zscore_series(g["notificacoes"]).fillna(0)

    for c in columns:
        if c not in g.columns:
            g[c] = 0 if c in {"notificacoes", "zscore_notificacoes"} else ""
    return g[columns]


def arbovirus_summary(
    df: pd.DataFrame,
    populacao: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """
    Consolida dengue, zika, chikungunya e demais arboviroses do SINAN.

    Produz série diária municipal por agravo com:
    - notificações
    - incidência por 100 mil (quando houver população)
    - z-score e EWMA
    - flag de aumento epidêmico
    """
    columns = [
        "data",
        "cod_ibge",
    ...
    base = sinan_summary(df)
    if base.empty:
        return pd.DataFrame(columns=columns)

    arbo = base[base["grupo_agravo"].eq("arbovirose")].copy()
    if arbo.empty:
        return pd.DataFrame(columns=columns)
    ...
    if populacao is not None and not populacao.empty:
        ...
        if "cod_ibge" in pop.columns:
            pop["cod_ibge"] = _normalize_cod_ibge(pop["cod_ibge"])
        pop_col = None
        for c in ["populacao", "populacao_2025", "pop", "populacao_estimada"]:
            if c in pop.columns:
                pop_col = c
                break
        if pop_col and "cod_ibge" in pop.columns:
            pop = (
                pop[["cod_ibge", pop_col]]
                .rename(columns={pop_col: "populacao"})
                .drop_duplicates("cod_ibge")
            )
            pop["populacao"] = _to_number(pop["populacao"], 0)
            arbo = arbo.merge(pop, on="cod_ibge", how="left")
        else:
            arbo["populacao"] = np.nan
    else:
        arbo["populacao"] = np.nan

    arbo["incidencia_100k"] = np.where(
        _to_number(arbo["populacao"], 0) > 0,
        _to_number(arbo["notificacoes"], 0) / _to_number(arbo["populacao"], 0) * 100_000,
        np.nan,
    )

    value_col = "incidencia_100k" if arbo["incidencia_100k"].notna().any() else "notificacoes"
    fill_series = arbo[value_col].fillna(arbo["notificacoes"])
    arbo["_serie"] = fill_series

    mcols = _municipality_key(arbo)
    group_cols = mcols + (["agravo"] if "agravo" in arbo.columns else [])
    sort_cols = [c for c in (group_cols + ["data"]) if c in arbo.columns]
    if sort_cols:
        arbo = arbo.sort_values(sort_cols)

    if group_cols:
        arbo["zscore_arbovirus"] = arbo.groupby(group_cols, group_keys=False)["_serie"].transform(
            lambda s: zscore_series(s).fillna(0)
        )
        arbo["ewma_arbovirus"] = arbo.groupby(group_cols, group_keys=False)["_serie"].transform(ewma)
        arbo["cusum_arbovirus"] = arbo.groupby(group_cols, group_keys=False)["_serie"].transform(
            simple_cusum
        )
    else:
        arbo["zscore_arbovirus"] = zscore_series(arbo["_serie"]).fillna(0)
        arbo["ewma_arbovirus"] = ewma(arbo["_serie"])
        arbo["cusum_arbovirus"] = simple_cusum(arbo["_serie"])

    arbo["alerta_aumento"] = (
        (arbo["zscore_arbovirus"] >= 1.5) | (arbo["cusum_arbovirus"] >= 3.0)
    ).astype(int)

    arbo = arbo.drop(columns=["_serie", "grupo_agravo", "zscore_notificacoes"], errors="ignore")
    for c in columns:
        if c not in arbo.columns:
            arbo[c] = 0 if c not in {"data", "cod_ibge", "municipio", "agravo"} else ""
    return arbo[columns]


def arbovirus_municipal_latest(df: pd.DataFrame, window_days: int = 7) -> pd.DataFrame:
    """
    Snapshot municipal recente de arboviroses para o classificador de estágio.

    Agrega a janela mais recente (padrão 7 dias) por município e devolve
    indicadores totais e por dengue/zika/chikungunya.
    """
    columns = [
        "cod_ibge",
        "municipio",
    ...
    arbo = df.copy()
    needed = {"agravo", "notificacoes", "zscore_arbovirus"}
    if not needed.issubset(set(arbo.columns)):
        arbo = arbovirus_summary(df)
    if arbo.empty:
        return pd.DataFrame(columns=columns)

    arbo["data"] = pd.to_datetime(arbo["data"], errors="coerce")
    arbo = arbo.dropna(subset=["data"])
    if arbo.empty:
        return pd.DataFrame(columns=columns)

    max_date = arbo["data"].max()
    start = max_date - pd.Timedelta(days=max(1, int(window_days) - 1))
    win = arbo[arbo["data"].between(start, max_date)].copy()
    if win.empty:
        return pd.DataFrame(columns=columns)

    win["notificacoes"] = _to_number(win["notificacoes"], 0)
    keys = [c for c in ["cod_ibge", "municipio"] if c in win.columns]
    if not keys:
        return pd.DataFrame(columns=columns)

    total = win.groupby(keys, as_index=False).agg(
        casos_arbovirus_7d=("notificacoes", "sum"),
        data=("data", "max"),
    )
    ...
    for label, col in [
        ("Dengue", "casos_dengue_7d"),
        ("Zika", "casos_zika_7d"),
        ("Chikungunya", "casos_chikungunya_7d"),
    ]:
        if label in pivot.columns:
            pivot = pivot.rename(columns={label: col})
        else:
            pivot[col] = 0

    named = {"casos_dengue_7d", "casos_zika_7d", "casos_chikungunya_7d"}
    other_cols = [
        c for c in pivot.columns
        if c not in set(keys) | named and str(c) in ARBOVIRUS_CANONICAL
    ]
    pivot["casos_outras_arbovirus_7d"] = pivot[other_cols].sum(axis=1) if other_cols else 0
    ...
    out = total.merge(pivot[keep_pivot], on=keys, how="left")

    agg_map: dict[str, tuple[str, str]] = {}
    if "zscore_arbovirus" in arbo.columns:
        agg_map["zscore_arbovirus"] = ("zscore_arbovirus", "max")
    if "incidencia_100k" in arbo.columns:
        agg_map["incidencia_arbovirus_100k"] = ("incidencia_100k", "max")
    if "alerta_aumento" in arbo.columns:
        agg_map["alerta_arbovirus"] = ("alerta_aumento", "max")

    if agg_map:
        z = arbo.groupby(keys, as_index=False).agg(**agg_map)
        out = out.merge(z, on=keys, how="left")
    else:
        out["zscore_arbovirus"] = 0.0
        out["incidencia_arbovirus_100k"] = np.nan
        out["alerta_arbovirus"] = 0

    if "agravo" in win.columns:
        dom = (
            win.groupby(keys + ["agravo"], as_index=False)["notificacoes"]
            .sum()
            .sort_values("notificacoes", ascending=False)
            .groupby(keys, as_index=False)
            .first()
            .rename(columns={"agravo": "agravo_dominante"})
        )
        out = out.merge(dom[keys + ["agravo_dominante"]], on=keys, how="left")
    else:
        out["agravo_dominante"] = ""

    out["data"] = pd.to_datetime(out["data"], errors="coerce").dt.date.astype(str)
    for c in columns:
        if c not in out.columns:
            out[c] = 0 if c not in {"data", "cod_ibge", "municipio", "agravo_dominante"} else ""
    return out[columns]
