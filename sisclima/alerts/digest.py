...
def format_municipal_telegram(p: dict[str, Any], *, cuiaba: bool = False) -> str:
    """Boletim municipal (inclui pacote dedicado Cuiabá) no padrão IndicaSUS + SISREG."""
    ...
    if cuiaba:
        lines.append(
            f"Remetente: {p.get('remetente') or 'ARARAS MT · Cuiabá'} · Regional: {p.get('regional') or '—'}"
        )
        lines.append("")
    ...
    footer = (
        "Relatório dedicado ARARAS MT · Cuiabá · CIEVS-MT / SES-MT."
        if cuiaba
...
def _select_payloads(payloads: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    Piloto restrito: com ``ALERT_MAX_MUNICIPIOS=0`` ou ``ALERT_FORCE_ONLY=true`` e
    ``ALERT_FORCE_MUNICIPIOS`` preenchido, envia **somente** os IBGE forçados
    (além de SES/Cuiabá conforme ``ALERT_LAYERS``) — sem ranking estadual.
    """
    ...
    force_only = as_bool(env("ALERT_FORCE_ONLY", "false"), False) or (max_mun == 0 and bool(force_ids))
    ...
    if ("regionais" in layers or "regional" in layers):
        ...
        if force_only and force_ids:
            # Regionais dos municípios forçados (ex.: Baixada Cuiabana / Teles Pires no piloto)
            mun_force = [
                p for p in payloads
                if p.get("escopo") == "municipal" and _ibge_tokens(p.get("alvo_id")) & force_ids
            ]
            reg_names = {
                str(p.get("regional") or p.get("regional_saude") or "").strip()
                for p in mun_force
                if str(p.get("regional") or p.get("regional_saude") or "").strip()
            }
            regionais = [p for p in regionais if str(p.get("alvo_nome") or "").strip() in reg_names]
    ...
    if "municipais" in layers or "municipal" in layers:
        municipais_all = [p for p in payloads if p.get("escopo") == "municipal"]
        forced = [p for p in municipais_all if _ibge_tokens(p.get("alvo_id")) & force_ids]
        # Ranking municipal: evita duplicar Cuiabá se a camada dedicada estiver ativa.
        municipais = list(municipais_all)
