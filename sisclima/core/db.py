...
def write_df(df: pd.DataFrame, table: str, if_exists: str = "replace") -> None:
    ...
    engine = get_engine()
    # SQLite limita variáveis por INSERT (~32k). Com 200+ colunas, chunksize em linhas
    # precisa caber: nrows_chunk * ncols <= 900 (conservador).
    ncols = max(1, len(df.columns))
    row_chunk = max(1, min(2000, 900 // ncols))
...
""",
    """
CREATE TABLE IF NOT EXISTS eventos_saude_notificacao (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
