...
ENV_BY_KEY = {
    ...
    'cemaden_alertas_csv': 'CEMADEN_ALERTAS_CSV',
...
def load_all_inputs() -> dict[str, pd.DataFrame]:
    ...
    try:
        out['cemaden_alertas'] = load_csv('cemaden_alertas_csv', ['data', 'data_emissao'])
    except Exception:
        out['cemaden_alertas'] = pd.DataFrame()
    try:
        out['sentinela_sg_agregado'] = load_csv('sentinela_sg_agregado_csv', [])
    except Exception:
        out['sentinela_sg_agregado'] = pd.DataFrame()
    try:
