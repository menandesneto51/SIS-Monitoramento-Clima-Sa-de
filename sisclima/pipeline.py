...
from sisclima.ingestion.ibge_municipios import get_municipios_operacionais, alinha_recorte_oficial
...
from sisclima.engines.recommendations import recommendations_for_context
...
def _inmet_municipio_has_alert(alerts: pd.DataFrame, municipio: str | None) -> str | None:
    ...
    def _blob(frame: pd.DataFrame) -> str:
        # astype(str) em DataFrame misto ainda pode deixar float/NaN no ravel
        vals = [str(v) for v in frame.tail(5).to_numpy().ravel().tolist()]
...
def _build_municipal_summary(met_ind, press, cap_agg, stock, infra, busca, com, sivep, lacen, sim, rumors, aq, vuln, inmet_alerts, arbo=None, cemaden_alerts=None, ana_risco=None, hidro_risco=None) -> pd.DataFrame:
    # Universo oficial IBGE (142). Indicadores entram por left-join; município sem dado permanece no recorte.
    oficial = get_municipios_operacionais()
    base = pd.DataFrame()
    if oficial is not None and not oficial.empty and "cod_ibge" in oficial.columns:
        cols = [c for c in ["cod_ibge", "municipio", "lat", "lon", "indice_vulnerabilidade_calor", "populacao", "regional_saude"] if c in oficial.columns]
        base = oficial[cols].drop_duplicates("cod_ibge").copy()
    if base.empty:
...
def run_pipeline(send_alerts: bool = True) -> dict:
    ...
    try:
        ...
        # Qualidade do ar: Copernicus/CAMS; se desligado ou vazio, Open-Meteo (PM2,5/IQA).
        ...
        if aq_raw.empty:
            prev = read_table('qualidade_ar_municipal')
            if prev is not None and not prev.empty:
                log.warning('Qualidade do ar: mantido último snapshot (fonte live vazia).')
                aq_raw = prev
        ...
        with db_conn() as conn:
            ...
            for eixo, rec in recommendations_for_context(str(indicador_row.get("nivel", "verde")), resumo_mun):
                execute(
                    conn,
