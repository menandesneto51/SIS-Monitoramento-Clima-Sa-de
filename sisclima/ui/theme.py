...
def apply_theme() -> None:
    ...
    st.markdown(
        ...
        f'margin:0 0 6px 0;opacity:.85;">{html.escape(DEVELOPER_CREDIT)} · Rede CIEVS · Vigidesastres</div>',
        unsafe_allow_html=True,
    )
...
def ses_footer() -> None:
    """Rodapé institucional SES-MT com logo CIEVS e crédito de desenvolvimento."""
    cievs_uri = image_data_uri(CIEVS_MT_LOGO_PATH)
    rede_uri = image_data_uri(REDE_CIEVS_LOGO_PATH)
    vigi_uri = image_data_uri(VIGIDESASTRES_LOGO_PATH)
    logo_block = ""
    if cievs_uri:
        logo_block += (
            f'<img src="{cievs_uri}" alt="CIEVS-MT" '
            f'style="height:42px;width:auto;display:block;margin-bottom:6px;" />'
        )
    badges = ""
    if rede_uri:
        badges += f'<img src="{rede_uri}" alt="Rede CIEVS" style="height:22px;width:auto;margin-right:10px;" />'
    if vigi_uri:
        badges += f'<img src="{vigi_uri}" alt="Vigidesastres" style="height:22px;width:auto;" />'

    st.markdown(
        f"""
        <div class="ses-footer">
          <div class="row">
            <div style="min-width:220px;">
              {logo_block}
              <strong>{html.escape(DEVELOPER_CREDIT)}</strong><br/>
              <span style="opacity:.92;">{html.escape(DEVELOPER_CREDIT_LONG)}</span>
            </div>
            <div>
            ...
            <div>
              <strong style="font-weight:800;">{html.escape(SYSTEM_NAME)} · Sala de situação</strong><br/>
              Uso interno · validar antes de comunicação oficial<br/>
              <a href="https://www.saude.mt.gov.br/ouvidoria" target="_blank" rel="noopener">Ouvidoria</a>
