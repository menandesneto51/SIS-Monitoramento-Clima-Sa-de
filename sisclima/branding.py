...
DEVELOPER_CREDIT = "Desenvolvido pelo CIEVS · SES-MT"
DEVELOPER_CREDIT_LONG = (
    f"{DEVELOPER_CREDIT} · {SYSTEM_NAME} ({SYSTEM_EXPANSION})"
)
PROJECT_DESCRIPTION = (
...
GOV_SES_LOCKUP_PATH = BRAND_ASSETS / "governo-ses-mt.png"
...
def image_data_uri(path: Path, *, max_bytes: int = 800_000) -> str | None:
    """Codifica imagem local para uso inline em HTML (rodapé/cabeçalho)."""
    try:
        if not path.exists():
            return None
        raw = path.read_bytes()
